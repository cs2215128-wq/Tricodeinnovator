...
-- Compatible with: Replit Postgres, Supabase, Neon, AWS RDS, Local PostgreSQL
-- Requires: pgvector extension (for vector(768) embeddings)
-- ============================================================================
...
-- 4. Uploaded Sources Table (Supports 11 Ingestion Modalities)
CREATE TABLE IF NOT EXISTS uploaded_sources (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    source_type VARCHAR(50) NOT NULL, -- 'pdf', 'slides', 'audio', 'youtube', 'arxiv', 'pubmed', etc.
...
-- 5. Source Embeddings Table (pgvector 768 dimensions for Gemini text-embedding-004)
...
-- 8. Active Quiz & Gamified Learning Sessions Table
...
-- 9. Academic Schedules & Study Planner Table
CREATE TABLE IF NOT EXISTS academic_schedules (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    title VARCHAR(255) NOT NULL,
    event_type VARCHAR(50) NOT NULL, -- 'lecture', 'deadline', 'study', 'exam'
